...
/*
The SQL a request is answered with: the columns a module serves (SELECTclause()),
how many rows match (SELECTcount()) and the conditions its filters come to
(WHEREclause(), which takes what generateParams() built). These are built from
what a module declares and from values the API has already escaped, and are
kept apart from the connection (core/database.php) so that what a request comes
to can be read, and tested, without a database.
*/
